...
folder_list = {
    ...
    'input': 'data\\REFIT\\after_culling\\%s\\1024\\1' % appliance_name,
}
...
Examples = collections.namedtuple("Examples", "inputs, targets, count,"
                                              "inputs_validation, targets_validation, count_validation, "
...
def metric(target_data, output_data, t_v, step):
    tar_new = []
    out_new = []
    mae = []
    ...
    if t_v == 'validation':
        with open('%s\\validation.txt' % folder_list['test_validation'], 'a+') as f:
...
def load_examples():
    ...
    ap_data = np.load(os.path.join(base_path, 'appliance_train_512.npy'))
    ag_data = np.load(os.path.join(base_path, 'main_train_1000.npy'))
    ap_data_validation = np.load(os.path.join(base_path, 'appliance_validation_512.npy'))
    ag_data_validation = np.load(os.path.join(base_path, 'main_validation_1000.npy'))
    ap_data_test = np.load(os.path.join(base_path, 'appliance_test_512.npy'))
    ag_data_test = np.load(os.path.join(base_path, 'main_test_1000.npy'))
    print('Data load complete!')

    appliance_data = tf.cast(ap_data, tf.float32)
    aggregate_data = tf.cast(ag_data, tf.float32)
    appliance_data_validation = tf.cast(ap_data_validation, tf.float32)
    aggregate_data_validation = tf.cast(ag_data_validation, tf.float32)
    ...
    queue_validation = tf.train.slice_input_producer([aggregate_data_validation, appliance_data_validation], shuffle=False)
    inputs_batch_validation, targets_batch_validation = tf.train.batch(queue_validation, batch_size=a.batch_size)
    ...
    return Examples(
        ...
        count=len(ap_data),
        inputs_validation=inputs_batch_validation,
        targets_validation=targets_batch_validation,
        count_validation=len(ap_data_validation),
...
def main():
    ...
    i_v = tf.reshape(examples.inputs_validation, [a.batch_size, 1024, 1, 1])
    t_v = tf.reshape(examples.targets_validation, [a.batch_size, 512, 1, 1])
    ...
    with sv.managed_session() as sess:
        ...
        for step in range(max_steps):
            ...
            if should(a.test_procedure):
                ...
                metric(target_test, output_test, 'test', results['global_step'])
                print('Test procedure complete!')

            if should(a.validation_procedure):
                print('Validation procedure!')
                target_validation.clear()
                output_validation.clear()
                max_steps_validation = examples.count_validation
                for step_validation in range(max_steps_validation):
                    inputs_, targets_ = sess.run([i_v, t_v])
                    data = sess.run({
                        'output': model.outputs,
                    }, feed_dict={
                        global_step: results['global_step'],
                    ...
                    for i in range(a.batch_size):
                        ...
                        for j in range(512):
                            tar_temp.append(int(_target[i][j] * scale))
                            out_temp.append(int(_output[i][j] * scale))
                        target_validation.append(tar_temp)
                        output_validation.append(out_temp)
                    if step_validation % 1000 == 0:
                        print('Process Validation: %f  Rate: %f' % (step_validation / max_steps_validation,
                                                                    (time.time() - start) / max_steps_validation))
                np.save('%s\\validation_target_%d.npy' % (folder_list['test_validation'], results['global_step']+1), target_validation)
                np.save('%s\\validation_output_%d.npy' % (folder_list['test_validation'], results['global_step']+1), output_validation)
                metric(target_validation, output_validation, 'validation', results['global_step'])
                print('Validation procedure complete!')
